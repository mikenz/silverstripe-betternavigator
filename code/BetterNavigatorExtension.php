...
class BetterNavigatorExtension extends DataExtension {
	...
	public function BetterNavigator() {
		...
		if($isDev || $canViewDraft) {
			...
			$editLink = false;
			if (isset($nav['CMSLink']) && isset($nav['CMSLink']['Link']) && (
				 ($this->owner->dataRecord->canEdit() && Permission::check('CMS_ACCESS_CMSMain')) || $isDev
				)) {
			...
			$bNData = array_merge($nav, array(
				...
				'LoginLink' => Controller::join_links('/', Config::inst()->get('SilverStripe\\Security\\Security', 'login_url'), $backURL),
